...
from Objective_gradient_utils import Objective_based_Training
...
def fine_tune_EWC_acuumelation_sparce(dataset_path,previous_task_model_path,init_model_path,exp_dir,data_dir,reg_sets,reg_lambda=1,norm='', num_epochs=100,lr=0.0008,batch_size=200,after_freeze=1,lam=0,wfc=False,b1=True,weight_decay=5e-4,head_shared=False,neuron_omega=False,metric='avg'):
    ...
    if not wfc:
        optimizer_ft = Objective_based_Training.Weight_Regularized_SGD(model_ft.parameters(), lr, momentum=0.9,weight_decay=weight_decay)
    else:
        optimizer_ft = Objective_based_Training.Weight_Regularized_SGD_FC(model_ft.parameters(), lr, momentum=0.9,weight_decay=weight_decay)
    ...
    model_ft = Objective_based_Training.train_model_sparce(model_ft, criterion, optimizer_ft,exp_lr_scheduler, lr,dset_loaders,dset_sizes,use_gpu,num_epochs,exp_dir,resume,lam=lam)
