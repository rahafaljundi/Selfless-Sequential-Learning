...
class Weight_Regularized_SGD(optim.SGD):
    ...
    def step(self, reg_params,closure=None):
        ...
        return loss
#from  torch.optim import Optimizer, required
class Weight_Regularized_SGD_FC(optim.SGD):
    r"""Implements stochastic gradient descent (optionally with momentum).
    Nesterov momentum is based on the formula from
    `On the importance of initialization and momentum in deep learning`__.
    Args:
    ...
    def __init__(self, params, lr=0.001, momentum=0, dampening=0,
                 weight_decay=0, nesterov=False,orth_reg=False):
        
        super(Weight_Regularized_SGD_FC, self).__init__(params, lr,momentum,dampening,weight_decay,nesterov)
        self.orth_reg=orth_reg
    def __setstate__(self, state):
        super(Weight_Regularized_SGD_FC, self).__setstate__(state)
            ...
            for p in group['params']:
                ...
                if p in reg_params:
                    ...
                    if p.data.ndimension()==2:
                        ...
                        regulizer=weight_dif.mul(4*reg_lambda*omega)#last layer weighted
                    else:
                        regulizer=weight_dif.mul(2*reg_lambda*omega)
                    del weight_dif
                    del curr_wegiht_val
...
def orth_org_hook(param,opt={}):
    if (len(param.size())==4):#conv2d
        ...
        #weight=weight.add(grad*coef) 
        #weight=weight.view(module.weight.size())
...
#from  torch.optim import Optimizer, required
#from  torch.optim import Optimizer, required
class Objective_Vector_Grad_After_SGD(optim.SGD):
    r"""Implements stochastic gradient descent (optionally with momentum).
    Nesterov momentum is based on the formula from
    `On the importance of initialization and momentum in deep learning`__.
    Args:
    ...
    def __init__(self, params, lr=0.001, momentum=0, dampening=0,
        ...
        super(Objective_Vector_Grad_After_SGD, self).__init__(params, lr,momentum,dampening,weight_decay,nesterov)
        
    def __setstate__(self, state):
        super(Objective_Vector_Grad_After_SGD, self).__setstate__(state)
